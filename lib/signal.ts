// ⚠️ 2026-08-10 현재 파이프라인에서 분리됨 — 어디서도 import하지 않는다.
// 사용자가 브리핑 메시지에서 TQQQ 신호를 "일단" 빼기로 해서 runAnalysis에서 호출만 제거했고,
// 로직 자체는 검증된 상태라 되살릴 수 있게 파일은 남겨둔다. 다시 넣으려면 lib/pipeline.ts의
// runAnalysis에 computeTqqqSignal을 추가하고 composeBrief 프롬프트에 섹션을 복원하면 된다.
//
