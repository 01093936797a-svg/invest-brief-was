...
계좌별: ${portfolio.byAccount.map((a) => `${a.account} ${won(a.value)}원(${a.pct.toFixed(1)}%)`).join(", ") || "확인 안 됨"}
등락 큰 종목: ${portfolio.movers.slice(0, 3).map((m) => `${m.name}(${m.account}) ${sgn(m.dayPct)}${m.dayPct.toFixed(2)}% [${rel(m.asOf)} 기준]`).join(", ") || "없음"}
누적 손익 상위: ${
    [...portfolio.holdings]
      .filter((h) => Math.abs(h.gainDiff) >= 100_000)
      .sort((a, b) => b.gainDiff - a.gainDiff)
      .slice(0, 2)
      .map((h) => `${h.name}(${h.account}) ${sgn(h.gainDiff)}${won(h.gainDiff)}원(${h.gainPct.toFixed(1)}%)`)
      .join(", ") || "해당 없음"
  }
누적 손익 하위: ${
    [...portfolio.holdings]
      .filter((h) => h.gainDiff < -100_000)
      .sort((a, b) => a.gainDiff - b.gainDiff)
      .slice(0, 2)
      .map((h) => `${h.name}(${h.account}) ${sgn(h.gainDiff)}${won(h.gainDiff)}원(${h.gainPct.toFixed(1)}%)`)
      .join(", ") || "해당 없음"
  }
...
■ 내 자산
- 총 …원 · 전일 대비 …원(…%) · 평가손익 …%
- 어디서 왔나: [기여도 분해] 인사이트를 금액으로 풀어 쓴다. 등락률만 쓰지 말고 몇 원인지 밝힌다
- 계좌별: 큰 계좌 2~3개만 "신한 ISA 3,200만(45%)" 형태로. 매일 전부 나열하지 않는다

📰 주요 헤드라인
- (아래 [참고 헤드라인]에서 3~5개를 그대로 옮긴다. 헤드라인 데이터가 아예 없을 때만 이 섹션을 생략한다)

💡 판단: (2~3문장. 간밤 시장→내 자산 연결. 위에서 이미 쓴 숫자를 다시 인용하지 말고, 숫자들이 함께 의미하는 바를 쓴다)
오늘 볼 것: (오늘 국장에서 주목할 포인트 한 줄. 없으면 "특이사항 없음" — 이 줄은 생략하지 말 것)`;
...
💡 판단: (2~3문장. 오늘 결과→오늘 밤 볼 것 연결. 헤드라인을 근거로 삼지 말 것)`;
...
- **같은 숫자를 두 번 쓰지 마라.** 지난 브리핑에서 나스닥 등락률이 한 줄 요약·시장 섹션·판단에 세 번 나왔는데, 세 번째부터는 정보가 아니라 자리만 차지한다. 어떤 수치든 가장 적합한 섹션 한 곳에서만 쓰고, 다른 섹션에서는 그 숫자를 전제로 다음 얘기를 해라.
- **판단 섹션은 위 내용의 요약이 아니다.** "미국 지수 상승이 내 나스닥 자산에 긍정적으로 작용했다" 같은 문장은 이미 위에 다 나온 말이라 쓸모가 없다. 숫자들을 합쳐야 나오는 것(비중 쏠림, 기여도 편중, 정책과의 괴리 등)이나, 정말 없으면 "오늘은 특별히 판단할 거리 없음"이라고 짧게 끝내라.
- 형식에 있는 섹션은 임의로 빼지 마라. 쓸 내용이 없으면 "특이사항 없음"이라고 채우되 줄 자체는 남긴다.
