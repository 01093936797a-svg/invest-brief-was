...
- 시장 섹션은 **내 보유종목에 실제로 영향을 준 것만** 최대 3줄. 지수·환율이 움직였어도 내 자산과 연결이 안 되면 그냥 뺀다. 일반 뉴스 요약지가 아니다.
- 한 줄 요약에 이미 쓴 숫자·표현은 아래 섹션에서 반복하지 않는다(예: 요약에 "휴장"이라 썼으면 시장 섹션에서 또 쓰지 않는다).
- 전체 12~15줄, 아침 30초 안에 읽히게. 지시("사라/팔아")가 아니라 관찰·양면 제시로.
...
■ 내 자산
- 총 …원 · 전일 대비 …원(…%) · 평가손익 …%
- 움직인 종목: 종목명(계좌) ±…% — 최대 3개. 휴장일이면 "휴장 — 직전 거래일 기준"이라 밝힌다. 움직임이 미미하면 "큰 움직임 없음"
- 배분: 주식 …% / 현금 …% / 코인 …% (목표와 눈에 띄게 어긋날 때만 한 마디 덧붙임)
- TQQQ 신호: 판정 한 줄 (STAY면 여기서 끝. 추정치 경고 있으면 "현금 X%(추정)"처럼 짧게)

■ 시장
- (내 보유에 실제 영향 준 것만, 최대 3줄)

💡 판단: (2~3문장. 시장→내 자산 연결, 관련 있을 때만 life-stage 프레임 한 스푼)
할 일: (실제 보유종목·신호 기준으로 구체적으로. 없으면 "없음 — 유지")
${dashboardUrl ? `\n🔗 대시보드: ${dashboardUrl}` : ""}

단, TQQQ 신호가 BUY나 SELL이면 "■ 내 자산" 아래에 별도 블록으로 승격해서 판정·사유·현금비중·실행규모를 쓴다(STAY일 때만 한 줄로 접는다).
