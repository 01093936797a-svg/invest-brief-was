...
**The two calls run on different models, deliberately** (as of 2026-08-11, after a ~$10-in-5-days bill). Cost here is dominated almost entirely by `researchMarket`: web search results accumulate in the conversation and the whole thing is re-sent on every iteration, so it's a huge-input / little-hard-reasoning workload. It runs on **`claude-sonnet-5`** at `effort: "low"` with `max_uses` on the search tool — low effort cuts tool calls as well as thinking tokens, and each search avoided removes its results from every later iteration's input. `composeBrief` is pure formatting and runs on **`claude-haiku-4-5`**. Roughly $0.50 → $0.20 per run.

**`max_uses` has a floor set by what the prompt asks for, and going under it fails invisibly.** It was first set to 4, but `researchMarket`'s task list covers at least five topics (국장 마감 / 미장 일정 / 선물 / 환율 / 코인), so the 2026-08-12 evening run burned its four searches and filled every market section with "확인 불가". That brief looked completely normal — correct portfolio numbers, right format, no thrown error — so it went out as a success and read as "quiet news day" rather than "the system couldn't search". It's now 8, and `webSearchErrorCodes()` inspects the response for `max_uses_exceeded` (and any other search error) and prepends an explicit system-failure notice to the research text so `composeBrief` has to say so plainly. **If you lower `max_uses`, cut the task list in the same edit** — and never treat a market section full of "확인 안 됨" as a quiet day without checking the logs for that error line first.
