...
**`lib/news.ts` brings back headlines, but not causation (as of 2026-08-14).** RSS feeds are free, so the "why" gap left by dropping web search is partly fillable — but only partly, and the distinction is load-bearing. The feeds give *article titles from the same time window*, which is not the same thing as the reason a number moved, and treating one as the other is exactly the failure that made the search tool worth removing. `formatHeadlines()` therefore ships an explicit anti-causation block with every batch (don't write "…때문에", only mention a headline when it names something actually held, drop the section entirely when nothing matches, never infer beyond the title since the body was never fetched). That block is a contract, not decoration — `news.test.ts` asserts its presence so it can't be quietly edited away.

The first version of that guard over-corrected: it only allowed a headline through when it *directly named a holding*, which for a portfolio of index ETFs is almost never, so the section could never appear. The distinction that actually works is **display vs. interpretation** — headlines always get shown, as a plain 📰 섹션 of verbatim titles, and what stays banned is using them as causes or as evidence in the verdict. A section the format block doesn't list is a section the model has no reason to produce, so both `morningFormat` and `eveningFormat` name it explicitly. All feeds failing costs the section and nothing else.

**The headline section links to a digest page (`api/news.ts`, as of 2026-08-14).** The 📰 heading in the Slack message is a hyperlink to `/api/news?date=…&kind=…`, which renders the same articles with Korean summaries — foreign headlines translated. Three constraints shaped it:

- **No auth, therefore no portfolio data.** A Slack link opens in a browser that can't carry `CRON_SECRET`, so the page is public. It contains only public article titles and summaries; if the URL leaks nothing is lost. Adding holdings data to that page would be a security regression, not a feature.
- **Built at brief time, not at view time.** Re-fetching RSS on page load would show different articles than the brief referenced, breaking the one thing the link is for. `recordNewsDigest()` runs *before* the Slack send (a fast click otherwise hits an empty page) and stores into `news_digests`, keyed `(date, kind)`.
- **Summaries come from RSS `<description>` only** — the article body is never fetched, so `buildDigest`'s prompt forbids inventing facts beyond the title and excerpt, and an item with no excerpt says so instead of being padded. One extra Haiku call, ~$0.005.

The URL is derived from the request host rather than a new env var, so cron hits (production alias) and manual preview-deploy runs each get the right origin. `linkifyNewsHeading()` does the substitution in code rather than asking the model to emit the URL — a model that alters one character ships a broken link, and `slack-blocks.test.ts` pins the variants it must tolerate.

⚠️ `news_digests` also has to be created by hand from `supabase/schema.sql`. Until then the link simply won't appear (the digest never saves, so `buildBriefBlocks` gets a null URL).
...
**Making the daily brief substantial is a data problem, not a length problem (as of 2026-08-14).** The 8/14 morning brief was thin in a specific way: the Nasdaq figure appeared three times (summary, market section, verdict), the +1,030,124원 daily change had no breakdown so there was no way to see where it came from, account balances were absent despite tax-account optimization being the top-priority policy, and the format's "오늘 볼 것" line was silently dropped. So `contributionBreakdown` now splits the day's change into won amounts per holding (a percentage alone doesn't say whether 3.42% is ten thousand won or three hundred), `PortfolioSummary` carries `byAccount` plus per-holding `dayDiff`/`gainDiff`, and the prompt bans reusing a number across sections and forbids dropping format lines. Adding rules to `lib/insights.ts` is how this brief gets richer; adding adjectives to the prompt is not.
